function third_sum(test_input) {
    const splitted_input = test_input
        .split('\n')
        .filter(el => el !== '')
        .reduce((counter, curr_val, index, array) => {
            const numbers = curr_val.match(/\d+/gm)
            if (numbers) {
                const numbers_properties_list = numbers
                    .reduce((numbers_counter, curr_number) => {
                        const start = curr_val.indexOf(curr_number)
                        const end = start + curr_number.length - 1
                        numbers_counter.push({start, end, curr_number})
                        return numbers_counter
                    }, [])
                    .reduce((numbers_counter, curr_number_properties) => {
                        const { start, end, curr_number } = curr_number_properties

                        let is_right_number = false

                        const neighbours = {
                            left: curr_val[start - 1] ? curr_val[start - 1] : null,
                            right: curr_val[end + 1] ? curr_val[end + 1] : null,
                            top: array[index - 1] ? array[index - 1].slice(start, end + 1) : null,
                            bottom: array[index + 1] ? array[index + 1].slice(start, end + 1): null,
                            upper_left: array[index - 1] ? array[index - 1][start - 1] : null,
                            upper_right: array[index - 1] ? array[index - 1][end + 1] : null,
                            bottom_left: array[index + 1] ? array[index + 1][start - 1] : null,
                            bottom_right: array[index + 1] ? array[index + 1][end + 1] : null,
                        }

                        Object.values(neighbours)
                            .filter(el => el !== null && el !== undefined)
                            .forEach(el => {
                                if (el.match(/[^\s.]/gm)) is_right_number = true
                            })

                        if (is_right_number) {
                            numbers_counter.push(+curr_number)
                        }

                        return numbers_counter
                }, [])

                numbers_properties_list.forEach(el => counter += el)
            }
...
console.log(third_sum(test_input))
